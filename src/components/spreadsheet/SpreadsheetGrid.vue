...
// Setup pagination (50 rows per page)
const {
  currentPage,
  totalPages,
  totalItems,
...
function getPendingForField(orderId, fieldKey) {
  return (
    allPendingChanges.value.find((p) => {
      if (p.targetId !== orderId) return false;
      const pendingField = p.field;
      const normalizedPendingField = pendingField.replace(
        "dynamic_fields.",
        "",
      );
...
function orderHasPendingChanges(orderId) {
  return allPendingChanges.value.some(
    (p) => p.targetId === orderId && p.status === "pending",
  );
}
...
async function handleCellSave({ field, value, previousValue, orderId }) {
  ...
  try {
    ...
    await ordersStore.updateOrder(orderId, field, value, order.version);
    ...
    if (err.code === "version_conflict") {
      const col = visibleColumns.value.find((c) => c.key === field);
      conflictDialog.value = {
        ...
        yourValue: value,
        serverValue: err.serverValue,
        baseValue: previousValue,
        yourVersion: order.version,
...
// Handle version conflict resolution - use user's value
async function handleUseYours({ field, value, serverVersion }) {
  const orderId = conflictDialog.value.orderId;
  conflictDialog.value.isOpen = false;

  try {
    // Retry with the server's current version
    await ordersStore.updateOrder(orderId, field, value, serverVersion);
    highlightCell(orderId, field);
    emit("order-updated", { orderId, field, value, resolved: "yours" });
  } catch (err) {
    console.error("[SpreadsheetGrid] Conflict resolution failed:", err);
    // If still conflicting, reopen dialog
    if (err.code === "version_conflict") {
      conflictDialog.value.isOpen = true;
      conflictDialog.value.serverVersion = err.serverVersion;
      conflictDialog.value.serverValue = err.serverValue;
    }
  }
}

// Handle version conflict resolution - use server's value
function handleUseServer({ field, value }) {
  const orderId = conflictDialog.value.orderId;
  conflictDialog.value.isOpen = false;

  // Just close dialog, the real-time listener will update the UI
  highlightCell(orderId, field);
  emit("order-updated", { orderId, field, value, resolved: "server" });
}
...
function handleDeleteConfirm({ orderId, forceDelete }) {
  ordersStore
    .softDeleteOrder(orderId, forceDelete)
    .then(() => {
...
<template>
  <div class="spreadsheet-grid">
        ...
        <table class="w-full text-sm">
          <thead class="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th
                class="py-3 px-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider sticky left-0 bg-gray-50 z-10"
              >
                Type
              </th>
              <!-- ID column -->
              <th
                class="py-3 px-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider"
              >
                ID
              </th>
              <!-- Dynamic columns -->
              ...
              <!-- Actions column -->
              <th
                class="py-3 px-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider"
              >
                Actions
              </th>
            </tr>
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            >
              <!-- Row type label -->
              <td
                class="py-2 px-3 text-xs text-gray-500 whitespace-nowrap sticky left-0 z-10"
                :class="showDeleted ? 'bg-red-50' : 'bg-white'"
              >
                <span v-if="showDeleted" class="text-red-500 font-medium"
                  >Deleted</span
                >
              ...
              <!-- Dynamic columns -->
              <td v-for="col in visibleColumns" :key="col.key" class="p-0">
                <SpreadsheetCell
                  ...
                  :is-editable="
                    !showDeleted && getColumnPermission(col.key).editable
                  "
                  :requires-approval="
                    getColumnPermission(col.key).requiresApproval
                  "
                  ...
                  @save="
                    (payload) =>
                      handleCellSave({ ...payload, orderId: order.id })
                  "
                  @pending-created="
                    (payload) =>
                      handlePendingCreated({ ...payload, orderId: order.id })
                  "
                />
              </td>
              ...
              <td class="py-2 px-3 text-right">
                <DeleteRowConfirm
                  ...
                  :order-id="order.id"
                  :order-label="
                    getOrderFieldValue(order, 'customer_name') || order.id
                  "
    ...
    <VersionConflictDialog
      :is-open="conflictDialog.isOpen"
      :field="conflictDialog.field"
      :field-label="conflictDialog.fieldLabel"
      :your-value="conflictDialog.yourValue"
      :server-value="conflictDialog.serverValue"
      :base-value="conflictDialog.baseValue"
      :your-version="conflictDialog.yourVersion"
      :server-version="conflictDialog.serverVersion"
      @use-yours="handleUseYours"
      @use-server="handleUseServer"
