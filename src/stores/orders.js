...
export const useOrdersStore = defineStore('orders', () => {
  ...
  // State
  // ---------------------------------------------------------------------------
  const orders = ref(new Map())       // Map<orderId, Order>
  ...
  const activeOrders = computed(() => {
    return Array.from(orders.value.values()).filter(
      ...
      (snapshot) => {
        snapshot.docChanges().forEach((change) => {
          if (change.type === 'added' || change.type === 'modified') {
            orders.value.set(change.doc.id, {
              id: change.doc.id,
              ...convertTimestamps(change.doc.data()),
            })
          } else if (change.type === 'removed') {
            orders.value.delete(change.doc.id)
          }
        })
        loading.value = false
      },
  ...
  async function updateOrder(orderId, field, value, version) {
    ...
    try {
      await runTransaction(db, async (tx) => {
        ...
        if (!snap.exists()) throw { code: 'not-found', message: 'Order not found.' }
        ...
        if (snap.data().status === 'completed') {
          throw { code: 'completed_order', message: 'Cannot edit a completed order.' }
        }
      ...
      orders.value.set(orderId, previous)
      handleError(err)
      throw err
    }
  }
  ...
  async function softDeleteOrder(orderId) {
    ...
    const previous = { ...order }
    const now = new Date()
    orders.value.set(orderId, { ...order, deletedAt: now })
  ...
  async function recoverOrder(orderId) {
    ...
    const previous = { ...order }
    orders.value.set(orderId, { ...order, deletedAt: null, deletedBy: null })
