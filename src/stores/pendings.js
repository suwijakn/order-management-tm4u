...
export const usePendingsStore = defineStore("pendings", () => {
  ...
  const allPendings = ref([]); // All pending changes for dashboard indicators
  const loading = ref(false);
  const error = ref(null);

  let unsubscribeListener = null;
  let unsubscribeAllListener = null;
  ...
  // All pending items for dashboard indicators (visible to everyone)
  const allPendingItems = computed(() =>
    allPendings.value.filter((p) => p.status === "pending"),
  );
  ...
  const pendingsForTarget = computed(
    () => (targetId) => pendings.value.filter((p) => p.targetId === targetId),
  );

  // Pendings for a specific target + field combination
  const pendingForField = computed(
    () => (targetId, field) =>
      pendings.value.find(
        (p) =>
          p.targetId === targetId &&
  ...
  /**
   * Fetch ALL pending changes for dashboard indicators (visible to everyone).
   * This is separate from fetchPendings which filters by user role.
   */
  function fetchAllPendings() {
    if (unsubscribeAllListener) {
      unsubscribeAllListener();
      unsubscribeAllListener = null;
    }
    ...
    // Fetch all pending changes regardless of role
    const q = query(
      collection(db, "pending_changes"),
      orderBy("requestedAt", "desc"),
    );

    unsubscribeAllListener = onSnapshot(
      q,
      (snapshot) => {
        const newPendings = [];
        snapshot.forEach((docSnap) => {
          const data = docSnap.data();
          const pending = convertTimestamps({
            id: docSnap.id,
            ...data,
          });
          newPendings.push(pending);
        });
        allPendings.value = newPendings;
      },
      (err) => {
        console.error("[pendings] Error fetching all pendings:", err);
      },
    );
  }
  ...
  async function createPending({
    ...
    if (
      ...
      error.value =
        "Only Managers and Super Admins can create pending changes for cost entries.";
      throw new Error(error.value);
    }
  ...
  async function approvePending(pendingId) {
    ...
    if (!["manager", "super_admin"].includes(authStore.userRole)) {
      error.value =
        "Only Managers and Super Admins can approve pending changes.";
      throw new Error(error.value);
    }
  ...
  async function rejectPending(pendingId) {
    ...
    if (!["manager", "super_admin"].includes(authStore.userRole)) {
      error.value =
        "Only Managers and Super Admins can reject pending changes.";
      throw new Error(error.value);
    }
  ...
  /**
   * Stop the real-time listeners and clear state.
   */
  function cleanup() {
    if (unsubscribeListener) {
      unsubscribeListener();
